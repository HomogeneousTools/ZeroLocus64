...
import { Factor, canonicalize, decodeLabel, encodeLabel } from "../src/index.js";
...
test("multiple summands may use different bases", () => {
  ...
  const summands = [[[1], [0]], [[5], [0]], [[12], [0]]];
  const label = encodeLabel(factors, summands);
  assert.deepEqual(normalizeDecoded(decodeLabel(label)), normalizeDecoded(canonicalize(factors, summands)));
});

test("invalid Dynkin type rank pairs are rejected on encode", () => {
  assert.throws(
    () => encodeLabel([new Factor("G", 7, 64)], []),
    (error) => error instanceof Error && error.message.includes("invalid Dynkin type/rank pair")
  );
});

for (const label of ["I0.24", "lF", "y0_", "0A1H000", "0B1H000"]) {
